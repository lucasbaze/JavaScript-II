...
{
    let ticketPriceTotal = runners.reduce((total, curr) => {
...
// Problem 1
//The business needs to know how many shirts they need to order and how many of each size. Return that information in a new object
{
    let shirtHisto = runners
        .map(runner => runner.shirt_size)
        .reduce((total, shirt) => {
            if (shirt in total) {
                total[shirt]++;
            } else {
...
//runners by company
let groupAble = groupBy(runners);
console.table(groupAble('company_name'));
console.table(groupAble('shirt_size'));
console.table(groupAble('donation'));

// Problem 3
//The business needs to send out gifts based on the range of the donation size. Group first_names by donation size
let donationRanges = {
    small: [1, 10],
    medium: [11, 100],
    large: [101, 10000],
...
let rangeReducer = (array, ranges) => {
    const entries = Object.entries(ranges);
    let reduced = array.reduce((acc, curr) => {
        for (let range of entries) {
            if (curr.donation >= range[1][0] && curr.donation <= range[1][1]) {
                var key = range[0];
            }
        }
...
let companyDonations = rangeReducer(runners, donationRanges);
console.table(companyDonations);
